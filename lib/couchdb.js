...
CouchDB.prototype.destroyAll = function destroyAll(model, where, options, cb) {
  ...
  self.all(model, {where: where, limit: self.getLimit()}, {raw: true}, function(
    err,
    docs
  ) {
    if (err) return cb(err, null);
...
CouchDB.prototype.count = function count(model, where, options, cb) {
  ...
  var self = this;
  self.all(model, {where: where, limit: self.getLimit()}, {}, function(
    err,
    docs
  ) {
...
CouchDB.prototype.selectModel = function(model, options, migrate) {
  ...
  // in dbSwitching mode
  if ((!options || !options.db) && this.settings.dbSwitching) {
    new Error('No database provided in context with dbSwitching active');
  }
    ...
    if (k === 'and' || k === 'or' || k === 'nor') {
      if (Array.isArray(cond)) {
        cond = cond.map(function(c) {
          return self.buildSelector(model, mo, c, options);
        });
      }
      query['$' + k] = cond;
      delete query[k];
