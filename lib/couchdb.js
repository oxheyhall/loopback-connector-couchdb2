  ...
  mo.db.insert(self.toDB(model, mo, data), function(err, result) {
    ...
    if (err) {
      dblogger.log({
        level: 'info',
        message: {
          requestId: options.req ? options.req.id || null : null,
          method: 'insert',
          docId: null,
          model: model,
          db: mo.dbName,
...
CouchDB.prototype._buildQuery = function(model, idName, query, where, options) {
  ...
  Object.keys(where).forEach(function(k) {
    ...
    if (k === idName) {
      k = '_id';
      cond =
        typeof cond === 'object' || Array.isArray(cond) ? cond : cond.toString();
    }
    var spec = false;
    var options = null;
    ...
    if (rst.warning) {
      dblogger.log({
        ...
        message: {
          ...
          success: true,
          err: null,
          indexWarning: rst.warning,
          query: query,
          time: null,
        },
      });
      indexWarning('%j: %j: %j', rst.warning, mo.modelName, query);
    }
