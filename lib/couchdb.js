...
// Caching is enabled in the following methods:
// _insert, all, destroy, destroyAll, updateAll
// A redis instance is either found in the context or
// instantiated where available.
// If not available, the caching is ignored.
// If available, the useCache setting on the model is checked.
// Where caching is needed, the model settings determine the key and field
// format for a hash, ejs is required to render this.
// Where active, cache is:
// set for inserts and updates
// deleted for destruction
// queried for reads. Where a read fails to find
// a result in the cache, the database is checked, and the
// cache is set for the next read.
// Read caches are stored by hashing the query and
// filter params, since each individual query would have a different
// result. This means the cache is effective for common and frequent queries.

// Where a model is not set to useCache, individual document ids
// are still cached to facilitate relationship checks.
// cache key: relations:{portfolioId}:{modelName}
// cache field: {docId}
// cache value: '' (empty string)
...
const dblogger = { log: (log) => debug(log) };
...
function getRedisInstance(modelInstance, options) {
  ...
  // debug({modelInstanceSettings: modelInstance.settings, optionsPortfolioId: options.portfolioId, optionsCurrentUser: options.currentUser});
  if (modelInstance.settings && modelInstance.settings.useCache) {
    if (process.env.NODE_ENV !== 'test') {
      if (
        ...
        modelInstance.name === 'SystemRole' ||
        modelInstance.name === 'SystemMembership'
      ) {
        if (
          (modelInstance.name === 'Account' ||
            modelInstance.name === 'SystemRole' ||
...
CouchDB.prototype.all = function all(model, filter, options, cb) {
      ...
      redisDb.hget(
        ...
        (cacheErr, cacheResult) => {
          if (cacheResult && !cacheErr) {
            ...
            dbLog(start, 'info', logMessage, cacheResult, null, null, true);
  ...
  if (!getCachedResult) {
    self._findRecursive(mo, query, docs, include, options, function (
      ...
      if (err) {
        dbLog(start, 'error', logMessage, result ? result.docs ? result.docs.length : result : '', null, err, false);
        return cb(err, result);
      }
 ...
 * @param {Object} where The where filter
 */
CouchDB.prototype._buildQuery = function (
  model,
  idName,
  query,
  where,
...
CouchDB.prototype._selectOperator = function (op, cond, regex) {
  ...
  switch (op) {
    ...
    case 'inq':
      ...
      if (newQuery['$in'].length === 1) {
        newQuery = newQuery['$in'][0];
      }
      break;
    case 'nin':
      newQuery = {
