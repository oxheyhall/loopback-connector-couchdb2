 ...
 */
  ...
  CouchDB.prototype.migrateOrUpdateIndex = function(models, isMigrate, cb) {
    ...
    function autoUpdateOneModel(model, cb) {
      ...
      var mo = self.selectModel(model, self.options, true);
  ...
  /**
   * Add an index for modelView propery to indexes got from modelDef
 ...
 */
  CouchDB.prototype.createIndex = function(ddocName, indexName, fields, cb) {
    ...
    var database = self.settings.dbSwitching ? self.options.db || self.settings.database : self.settings.database;
    var requestObject = {
      db: database,
      path: '_index',
      method: 'post',
 ...
 */
  CouchDB.prototype.getModelIndexes = function(model, cb) {
    ...
    var self = this;
    var mo = self.selectModel(model, self.options);
